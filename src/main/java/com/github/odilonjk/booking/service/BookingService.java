...
public class BookingService {
    ...
    @Transactional
    public synchronized UUID createBooking(Booking bookingRequest) {
        var newId = UUID.randomUUID();
        if (repository.existsBookingToBeOverlapped(bookingRequest.getStartDate(), bookingRequest.getEndDate(), newId)) {
        ...
        var booking = repository.saveAndFlush(newBooking);
    ...
    public void updateBooking(UUID id, Booking updatedValues) {
        ...
        if (repository.existsBookingToBeOverlapped(updatedValues.getStartDate(), updatedValues.getEndDate(), id)) {
